...
/*pick subsequence from position l to u-1*/
int pick_sseq(unsigned char *sseq, unsigned char *sseq_num, int n, unsigned char *sseq_out, unsigned char *sseq_num_out, int l, int u)
{
    return;
}
...
/*read paths*/
    ...
    fscanf(seq_paths,"%s",Idxr);
    fscanf(seq_paths,"%s",Pr);
...
/*read filenames*/
    ...
    int nr;
    char **filenamesrand;
    nr = Lines_ReadFile(Idxr,&filenamesrand,NULL);
    int nq;
    char **queries;
...
/*Compute length of each chromesome*/
    ...
    int *cr;
    cr = (int*)malloc(sizeof(int)*nr);
    for(t=0;t<nr;t++){
	cr[t] = 0;
	for(i=0;i<lr[t];i++){
	    cr[t] += sseq_num_rand[t][i];
	}
    }
