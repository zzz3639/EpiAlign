...
int main(int argc, char **argv)
{
    ...
    score_fre = (float*)malloc(sizeof(float)*ns);
    ComputeScoreFre(sseq,ns,w,score_fre);
    float *final_score_sseq;
    final_score_sseq = (float*)malloc(sizeof(float)*ns);
    AttentionScore(sseq, sseq_num, ns, w, add_score, wn, final_score_sseq);

    fprintf(stdout,"\n");
    for(i=0;i<ns;i++){
	fprintf(stdout,"%c\t%d\n",State2Char(sseq[i]),sseq_num[i],score_fre[i],score_sseq[i],score_fre[i]*score_sseq[i],2.0*final_score_sseq[i]);
    }
    return 1;
}
